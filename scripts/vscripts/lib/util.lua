--[[
	*Library created by CryDeS.
	*You can uses this in you gamemode for free, without and any copyright
	...
	Functions:
	 ...
	 CDOTA_BaseNpc:HasTalent(talent_name)									// if basenpc has talent
	 CDOTA_BaseNpc:GetTalentSpecialValueFor(talent_name) 					// gets "value" param
]]
...
function Util:DisableSpellAmp(caster, damage)
	...
	return damage/damage_int_pct_add
end
...
function Util:_DealDamage(victim, attacker, damage_type, damage_const, damage_pct, from_max_health)
	...
	damage = (damage + damage_const) / damage_int_pct_add
	--damage = damage / damage_int_pct_add

	print("[UTIL] _DealDamage deal [" .. damage_before_spellamp .. "] damage before spellamp, [".. damage.. "] after spell amp")
	local damage_table = {
		victim 		= victim,
		attacker 	= attacker,
		damage 		= damage,
...
function CDOTA_BaseNPC:GetTalentSpecialValueFor(talent_name)
    if self:HasAbility(talent_name) and self:FindAbilityByName(talent_name):GetLevel() ~= 0 then 
    	return self:FindAbilityByName(talent_name):GetSpecialValueFor("value")
    end
