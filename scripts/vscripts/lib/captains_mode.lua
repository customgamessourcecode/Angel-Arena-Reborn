...
local STAGE_TIMES = {
	...
	[2]  = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN,  		team = DOTA_TEAM_GOODGUYS, 	slot = 2 },
	[3]  = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN,  		team = DOTA_TEAM_BADGUYS, 	slot = 1 },
	[4]  = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN,  		team = DOTA_TEAM_BADGUYS, 	slot = 2 },

	[5]  = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_GOODGUYS, 	slot = 1 },
	[6]  = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_GOODGUYS, 	slot = 2 },

	[7]  = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_BADGUYS, 	slot = 1 },
	[8]  = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_BADGUYS, 	slot = 2 },

	[9]  = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN,  		team = DOTA_TEAM_GOODGUYS, 	slot = 3 },
	[10] = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_GOODGUYS, 	slot = 3 },

	[11] = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN,  		team = DOTA_TEAM_BADGUYS, 	slot = 3 },
	[12] = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_BADGUYS, 	slot = 3 },

	[13] = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN,  		team = DOTA_TEAM_GOODGUYS, 	slot = 4 },
	[14] = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN,  		team = DOTA_TEAM_BADGUYS, 	slot = 4 },
	[15] = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_GOODGUYS, 	slot = 4 },
	[16] = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK,  		team = DOTA_TEAM_BADGUYS, 	slot = 4 },

	[17] = { time = BAN_TIME, stage_type 		= ARENA_STAGE_BAN, 			team = DOTA_TEAM_GOODGUYS, 	slot = 5 },
	[18] = { time = PICK_TIME, stage_type 		= ARENA_STAGE_BAN, 			team = DOTA_TEAM_BADGUYS, 	slot = 5 },

	[19] = { time = BAN_TIME, stage_type 		= ARENA_STAGE_PICK, 		team = DOTA_TEAM_BADGUYS, 	slot = 5 },
	[20] = { time = PICK_TIME, stage_type 		= ARENA_STAGE_PICK, 		team = DOTA_TEAM_GOODGUYS, 	slot = 5 },
	
	[21] = { time = GAME_START_TIME, stage_type = ARENA_STAGE_UNDEFINED, 	team = DOTA_TEAM_BOTH, 		slot = 0 },
	[22] = { time = GAME_START_TIME, stage_type = ARENA_STAGE_UNDEFINED, 	team = DOTA_TEAM_BOTH, 		slot = 0 },
}
...
local STAGE_CONTROL  = {
	...
	[20] = 4,
	[21] = 2,
	[22] = 2, -- for some type idiotto errors :3
}]]
			...
			if PlayerResource:GetTeam(i) == DOTA_TEAM_GOODGUYS then
				for hero_name, _ in pairs(radiant_picks) do
					if not picked_heroes[hero_name] then
						print("PICK HERO", hero_name, " for pid", i)
...
--[[
	if stage == 0 then	
		if team_captains[DOTA_TEAM_GOODGUYS] == -1 then
		...
		if team_captains[DOTA_TEAM_BADGUYS] == -1 then
			for i = 5, 9 do
				if PlayerResource:IsValidPlayerID(i) and PlayerResource:GetConnectionState(i) == DOTA_CONNECTION_STATE_CONNECTED then
					CM:OnCaptainSelected( { playerID = i } )
					break;
...
function CM:_PickHero(playerid, hero_name, team)
	...
	--while(PlayerResource:GetSelectedHeroName(playerid) ~= hero_name) do
		--player:MakeRandomHeroSelection() 
	--end
	...
	print("createhero player")
	local hero = CreateHeroForPlayer(hero_name, PlayerResource:GetPlayer(playerid))

	if tHeroesRadiant then
